...
public class ChatRoomController {
     ...
     * @param memberId 현재 로그인한 Member ID (JWT에서 자동 추출)
     * @return 역할에 맞는 채팅방 목록
     */
    @Operation(
        summary = "채팅방 목록 조회",
        description = "Member의 현재 역할(PHOTOGRAPHER/CUSTOMER)에 따라 채팅방 목록을 반환합니다. "
    )
    @GetMapping
    public ResponseEntity<ChatRoomDto.ListResponse> getMyChatRooms(
            @Parameter(hidden = true) @AuthenticationPrincipal Long memberId
    ) {
        ChatRoomDto.ListResponse response = chatRoomService.getMyChatRooms(memberId);
