...
# 카메라 데이터(Default Camera)
- 픽플즈 기능정의 구글 스프레드시트의 '촬영 기기 목록'에 적힌 카메라들을 테이블에 수동으로 넣어둡니다.
- 클라이언트에서 조회해서 활용할 수 있도록 (기획이 바뀌어도 클라이언트 코드를 바꿀 필요 없도록) 하기 위함입니다.
- mysql에서 `default_camera_insert.sql`을 실행하시면 됩니다.